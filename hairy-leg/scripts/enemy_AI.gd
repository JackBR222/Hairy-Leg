...
# NOVO: OFFSET CONFIGURÁVEL PARA INVESTIGAÇÃO
@export var investigate_reach_distance: float = 1.2
...
func _physics_process(delta: float) -> void:
	...
	# NOVO UPDATE CONSTANTE: interrompe perseguição se o player ficar escondido durante o deslocamento
	_check_hidden_interrupt()

	if not player_is_hidden and _can_see_player() and state not in [State.CHASE, State.ATTACK]:
...
# NOVO: INTERRUPÇÃO CONSTANTE DE STEALTH DURANTE CHASE
...
func _state_attack() -> void:
	...
	var player = get_tree().current_scene.get_node("Player/Player")

	if player:
		player.freeze_input()

	var fade = get_tree().current_scene.get_node("CanvasLayer/FadeInOut")
